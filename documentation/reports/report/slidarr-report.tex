...
MIDI was designed to be used with digital piano keyboards. The messages indicate \textit{note on}, \textit{note off}, and their \textit{velocity} (volume). The protocol is built around the concept of piano notes, which is not optimal for the Slidarr which operates at a continous scale rather than at fixed frequency intervals.
...
As mentioned in secion \ref{sec:midi_protocol}, MIDI transmit fixed notes and not the actual frequency values. When touching the string, it is unlikely to hit at a perfect MIDI frequency. In order to be able to slide the frequency up or down continuously, the Slidarr calculates how much the touch position deviates from the actual MIDI note, and uses that value to send a \textit{pitchbend} message. 
...
The code can be made more modular for use on other microcontrollers, for Slidarr by converting it to use CMSIS. A more optimal and cost effective microcontroller can be choosen. The current program uses 12KB (out of 256KB) of flash storage and 500 bytes (out of 32KB) of SRAM. In addition the software runs without any problems at 16MHz. This means a much smaller microcontroller could be used for future implementations.
